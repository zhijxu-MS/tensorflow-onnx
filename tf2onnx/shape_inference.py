...
def infer_shape_for_node(g, node):
    ...
    if node.type == "Unpack":
        ...
        # from this statement num must equal to input_shape[axis]
    ...
    if node.type in ["Minimum", "Maximum"]:
        ...
        utils.make_sure(len(input_shapes) == 2, "ops only need two inputs")
        utils.make_sure(_broadcasting_rule_check(*input_shapes), "input can't be broadcasted")
